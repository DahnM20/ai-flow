...
class AsyncProcessorLauncher(AbstractTopologicalProcessorLauncher):
    ...
    def convert_processors_to_node_dict(self, processors: List[Processor]):
        ...
        for processor in processors.values():
            nodes[processor.name] = self.Node(processor.name, self.get_input_processor_names(processor), processor)
        return nodes
    ...
    def launch_processors(self, processors: List[Processor]):
        ...
        while nodes:
            for id, node in nodes.items():
                if node.state == AsyncProcessorLauncher.NodeState.PENDING and self.can_run(node, nodes):
