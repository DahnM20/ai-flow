...
from app.processors.factory.processor_factory_iter_modules  import ProcessorFactoryIterModules
...
class ProcessorFactoryMock(ProcessorFactoryIterModules):
    ...
    def create_processor(self, config, api_context_data=None, storage_strategy=None):
        ...
        def fake_process(*args, **kwargs):
            if self.with_delay:
                sleep_duration = random.uniform(ProcessorFactoryMock.MIN_DELAY, ProcessorFactoryMock.MAX_DELAY)
                eventlet.sleep(sleep_duration)
                
            if mock_processor.processor_type in ["dalle-prompt", "stable-diffusion-stabilityai-prompt"]:
                output = [self.fake_img_output] if self.fake_img_output is not None else ["https://ai-flow-public-assets.s3.eu-west-3.amazonaws.com/v0.4.0-sample-1.png"]
            elif mock_processor.processor_type in ["ai-data-splitter"]:
                output = self.fake_multiple_output if self.fake_multiple_output is not None else ["Lorem Ipsum", "Lorem Ipsum"]
            else : 
                output = self.fake_text_output if self.fake_text_output is not None else "Lorem Ipsum"
