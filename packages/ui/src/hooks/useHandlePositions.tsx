...
const useHandlePositions = (data: any, nbInput: number, outputHandleId: string) => {

    const allInputHandleIds = Array.from({ length: nbInput }, (_, i) => i).map((index) => generateIdForHandle(index));

    const allHandleIds = useMemo(() => {
        const inputHandleIds = Array.from({ length: nbInput }, (_, i) => i).map((index) => generateIdForHandle(index));
